...
/**
 * @fileoverview Helper functions for generating MicroPython for blocks.
...
Blockly.MicroPython.init = function(workspace) {
  ...
  Blockly.MicroPython.variableDB_.setVariableMap(workspace.getVariableMap());
...
Blockly.MicroPython.finish = function(code) {
  ...
  for (var name in Blockly.MicroPython.definitions_) {
    var def = Blockly.MicroPython.definitions_[name];
...
Blockly.MicroPython.scrub_ = function(block, code) {
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    for (var i = 0; i < block.inputList.length; i++) {
      if (block.inputList[i].type == Blockly.INPUT_VALUE) {
        ...
        if (childBlock) {
          var comment = Blockly.MicroPython.allNestedComments(childBlock);
          if (comment) {
            commentCode += Blockly.MicroPython.prefixLines(comment, '# ');
          }
        }
  ...
  var nextCode = Blockly.MicroPython.blockToCode(nextBlock);
 ...
 */
Blockly.MicroPython.getAdjustedInt = function(block, atId, opt_delta, opt_negate) {
  var delta = opt_delta || 0;
  if (block.workspace.options.oneBasedIndex) {
    delta--;
  }
  var defaultAtIndex = block.workspace.options.oneBasedIndex ? '1' : '0';
  var atOrder = delta ? Blockly.MicroPython.ORDER_ADDITIVE :
      Blockly.MicroPython.ORDER_NONE;
  var at = Blockly.MicroPython.valueToCode(block, atId, atOrder) || defaultAtIndex;
